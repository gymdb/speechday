...
class EventDAO extends AbstractDAO {
    public static function getEvents() {
        ...
        $res = self::query($con, 'SELECT id, name, dateFrom, dateTo, slotTimeMin, isActive FROM event ORDER BY dateFrom;', array());

        while ($e = self::fetchObject($res)) {
            $events[] = new Event($e->id, $e->name, $e->dateFrom, $e->dateTo, $e->slotTimeMin, $e->isActive);
        }
        self::close($res);
    ...
    public static function createEvent($name, $dateFrom, $dateTo, $slotDuration, $setActive) {
        $con = self::getConnection();
        ...
        self::query($con, 'INSERT INTO event (name, dateFrom, dateTo, slotTimeMin) VALUES (?, ?, ?, ?);', array($name, $dateFrom, $dateTo, $slotDuration));
    ...
    public static function getEventForId($eventId) {
        $event = null;
        $con = self::getConnection();
        $res = self::query($con, 'SELECT id, name, dateFrom, dateTo, slotTimeMin, isActive FROM event WHERE id = ?;', array($eventId));

        if ($e = self::fetchObject($res)) {
            $event = new Event($e->id, $e->name, $e->dateFrom, $e->dateTo, $e->slotTimeMin, $e->isActive);
        }
        self::close($res);
        return $event;
    }
    ...
    public static function getActiveEvent() {
        $event = null;
        $con = self::getConnection();
        $res = self::query($con, 'SELECT id, name, dateFrom, dateTo, slotTimeMin, isActive FROM event WHERE isActive = 1;', array());
    ...
    public static function getEventsForDate($unixDate) {
        ...
        $events = array();
        $con = self::getConnection();
        $res = self::query($con, 'SELECT id, name, dateFrom, dateTo, slotTimeMin, isActive FROM event WHERE dateFrom BETWEEN ? AND ?;', array($from, $to));
